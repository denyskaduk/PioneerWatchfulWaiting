...
FROM 
(--adding 180 days to the index date
  select pe.event_id, pe.person_id, DATEADD(day,180,pe.start_date) as start_date, pe.end_date, pe.op_start_date, pe.op_end_date, row_number() over (partition by pe.person_id order by pe.start_date ASC) as ordinal, cast(pe.visit_occurrence_id as bigint) as visit_occurrence_id
